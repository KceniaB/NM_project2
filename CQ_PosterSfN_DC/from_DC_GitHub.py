...
for (target, NM), df_target in df_unbiased.groupby(['target', 'NM']):
    for event in EVENTS:
        ...
        for ax, feedback in zip(axs, [1, -1]):
            ...
            ax.axhline(0, ls='--', color='gray')
            ax.axvline(0, ls='--', color='gray')
            ax.axvspan(*RESPONSE_WINDOW, color='gray', alpha=0.2)
            ax.set_xticks(np.linspace(PSTH_WINDOW[0], PSTH_WINDOW[1], 3))
            ax.set_xlabel('Time from event (s)')
            ax.set_ylabel('$\Delta$F / F')
            ax.legend(
...
""" KB adding Jan13 """
# Plot response magnitude for reaction time quartiles in the 50-50 block - copy and modification from above

#for relative_contrast and contrast (comment)
# Plot response magnitude for signed contrasts in the 50-50 block
df_unbiased = df_responses.query('p_left == 0.5').copy()
response_window = '-'.join([str(t) for t in RESPONSE_WINDOW])
plot_subjects = False
for (target, NM), df_target in df_unbiased.groupby(['target', 'NM']):
    ...
    for event in EVENTS:
        ...
        axs = [ax]
        fig.suptitle(f'{target}-{NM} - {event_name} ({response_window}s)', fontsize=25)
        # for ax, side in zip(axs, [True, False]):
        #     df_side = df_event.query('side == @side')
        for ax in axs:
            df_side = df_event
            ...
            for feedback, linestyle in zip([1, -1], ['-', '--']):
                # contrast_groups = df_side.query('feedback == @feedback').groupby('relative_contrast')
                contrast_groups = df_side.groupby('relative_contrast')
                sorted_groups = [contrast_groups.get_group(c) for c in contrasts]
                # ax.errorbar(
                #     np.arange(len(contrasts)),
                #     [group['response_mean'].mean() if len(group) > 10 else np.nan for group in sorted_groups],
            ...
            ax.set_xticks(np.arange(len(contrasts)))
            ax.set_xticklabels([f'{c*100:.0f}' for c in contrasts])
            ax.set_xlabel('Contrast level', fontsize=30)
            ax.axhline(0, ls='--', color='gray')
            # if side:
            #     ax.set_ylabel('$\Delta$F/F')
            # else:
        ...
        if plot_subjects:
            ...
            subject_means = df_event.groupby(
                # ['subject', 'relative_contrast', 'feedback', 'side']
                ['subject', 'contrast', 'feedback', 'side']
                )['centered_mean'].mean()
            y_min = subject_means.min()
            y_max = subject_means.max()
        else:
...
for (target, NM), df_target in df_unbiased.groupby(['target', 'NM']):
    ...
    for event in EVENTS:
        event_name = event.split("_")[0]
        df_event = df_target.query('event == @event').copy()
        # Canonicalize contrasts: merge -0 and 0, keep only -1, 0, +1
        df_event['relative_contrast_canon'] = (
            df_event['relative_contrast']
            .round(2)                 # safety for floats
            .replace({-0.0: 0.0})     # explicit -0 → 0
        )

        df_event = df_event.query(
            'relative_contrast_canon in [-1, 0, 1]'
        )
        ...
        for ax in axs:
            ...
            for feedback, linestyle in zip([1, -1], ['-', '--']):
                # contrast_groups = df_side.query('feedback == @feedback').groupby('relative_contrast')
                contrast_groups = df_side.groupby('relative_contrast_canon')
                sorted_groups = [contrast_groups.get_group(c) for c in contrasts]
                # ax.errorbar(
                #     np.arange(len(contrasts)),
                #     [group['response_mean'].mean() if len(group) > 10 else np.nan for group in sorted_groups],
...
#%%
"""
KB lineplots joined future correct and incorrect - to pick the stimOn figures
"""
...
for (target, NM), df_target in df_unbiased.groupby(['target', 'NM']):
    for event in EVENTS:
        ...
        for ax in axs:
            ...
            trial_groups = df_event.groupby('contrast')
            colors = NM_CMAPS[NM](np.linspace(0.3, 0.99, len(trial_groups)))
            for (contrast, trials), color in zip(trial_groups, colors):
                plot_mean_response(
                    trials,
            ...
            ax.axvspan(
                ...
                color='gray',
                alpha=0.15,
                zorder=0   # keeps it behind the traces
            )

            ax.axhline(0, ls='--', color='gray')
            ax.axvline(0, ls='--', color='gray', linewidth=1)
            # ax.axvspan(*RESPONSE_WINDOW, color='gray', alpha=0.15)
            ax.set_xticks(np.linspace(PSTH_WINDOW[0], PSTH_WINDOW[1], 3))
            ax.set_xlabel('Time from event (s)', fontsize=23)
            ax.set_ylabel('$\Delta$F / F', fontsize=23)
            ax.legend(
                title='Contrast', frameon=False, loc='upper left', bbox_to_anchor=(1, 1)
                )
...
"""
KB lineplots with contrasts merged 
15-Jan-2026
"""
# Plot responses by contrast for each target-NM in the 50-50 block
df_unbiased = df_responses.query('p_left == 0.5').copy()
...
""" 
KB lineplots split correct incorrect, but side merged 
""" 
...
for (target, NM), df_target in df_unbiased.groupby(['target', 'NM']):
    ...
    for event in EVENTS:
        ...
        for ax in axs:
            ...
            for feedback, linestyle in zip([1, -1], ['-', '--']):
                contrast_groups = (
                    df_side.query('feedback == @feedback')
                        .groupby('relative_contrast')
                )
                sorted_groups = [contrast_groups.get_group(c) for c in contrasts]

                ax.errorbar(
                    np.arange(len(contrasts)),
                    [group['response_mean'].mean() if len(group) > 10 else np.nan
                    for group in sorted_groups],
                    yerr=[group['response_mean'].sem() if len(group) > 10 else np.nan
                        for group in sorted_groups],
                    marker='o',
                    color=NM_COLORS[NM],
                    linestyle=linestyle,
