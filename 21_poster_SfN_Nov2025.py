#%%
##############################################################################
# KB code for the poster for SfN Nov2025
# ----------------------
...
for NM, subjects in MICE.items():
    ...
    # faded per-session lines
    for y in session_curves:
        ax.plot(xgrid, y, color=color, alpha=0.75, lw=1.2)

    # scatter dots per session
    for xc, pc in session_dots:
        ax.scatter(xc, pc, color=color, alpha=0.75, s=45, edgecolor="none")
    ...
    ax.set_xlabel("Signed contrast (left | right)")
    ax.set_ylabel("Proportion of right choices")
    ax.set_title(f"Psychometric curve for {NM}\n(N={sess_used} high-performance sessions)")
    ax.legend(frameon=False, loc="lower right")
    ...
    mean_line    = Line2D([0], [0], color="black", lw=8, label="Average psychometric fit")
...
def find_trials_file(base_dir, subject, date, region, eid):
    for f in os.listdir(base_dir):
        if f.startswith("df_trials_") and all(x in f for x in [subject, date, region, eid]) and f.endswith(".csv"):
            return os.path.join(base_dir, f)
    return None

def load_psth_and_trials(fpath):
    """Load psth .npy and matching df_trials."""
    fname = os.path.basename(fpath)
    parts = fname.replace("psth_", "").replace(".npy", "").split("_")
    if len(parts) < 6:
        return None, None, None
    subj, date, region, eid, probe, event = parts[:6]
    event = "stimOnTrigger_times" if "stimOn" in event else "feedback_times"
    tpath = find_trials_file(TRIALS_DIR, subj, date, region, eid)
    if not tpath:
        return None, None, None
    psth = np.load(fpath)
    df = pd.read_csv(tpath)
    n = min(psth.shape[1], len(df))
    return psth[:, :n], df.iloc[:n], event

# =========================================================
# GATHER DATA
# =========================================================
data = {nm: {"stimOn": {"correct": [], "incorrect": []},
             "feedback": {"correct": [], "incorrect": []}} for nm in MICE.keys()}

for nm, subjects in MICE.items():
    for f in os.listdir(PSTH_DIR):
        if not f.endswith(".npy") or not any(s in f for s in subjects):
            continue
        psth, df, event = load_psth_and_trials(os.path.join(PSTH_DIR, f))
        if psth is None: continue
        fb = df["feedbackType"].values
        try:
            corr = np.nanmean(psth[:, fb == 1], axis=1)
            inc = np.nanmean(psth[:, fb == -1], axis=1)
        except Exception:
            continue
        if "stimOn" in event:
            data[nm]["stimOn"]["correct"].append(corr)
            data[nm]["stimOn"]["incorrect"].append(inc)
        else:
            data[nm]["feedback"]["correct"].append(corr)
            data[nm]["feedback"]["incorrect"].append(inc)

# =========================================================
# PLOT (with resampling)
# =========================================================
TARGET_LEN = 90  # unify PSTH length
time = np.linspace(PERIEVENT_WINDOW[0], PERIEVENT_WINDOW[1], TARGET_LEN)

for event in ["stimOn", "feedback"]:
    fig, axes = plt.subplots(1, 2, figsize=(15, 9), sharey=True, dpi=300)
    for i, cond in enumerate(["correct", "incorrect"]):
        ax = axes[i]
        for nm, color in PALETTES.items():
            curves = [resample_to_target_length(a, TARGET_LEN)
                      for a in data[nm][event][cond]
                      if a is not None and np.isfinite(a).any()]
            if len(curves) == 0:
                continue
            arr = np.vstack(curves)
            mean = np.nanmean(arr, axis=0)
            err = sem(arr, axis=0, nan_policy="omit")
            ax.plot(time, mean, color=color, lw=3, label=nm)
            ax.fill_between(time, mean - err, mean + err, color=color, alpha=0.25)
        ax.axvline(0, color="gray", ls="--", lw=1)
        ax.set_title(f"{cond.capitalize()} trials")
        ax.set_xlabel("Time (s)")
        if i == 0:
            ax.set_ylabel("Neuromodulator activity")
        ax.set_ylim(YLIM)
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
    fig.suptitle(f"{event} — PSTH mean ± SEM", fontsize=13)
    fig.legend(PALETTES.keys(), loc="top right", ncol=4, frameon=False)
...
NM_ORDER = ["DA", "5-HT", "NE", "ACh"]
LEGEND_HANDLES = [Line2D([0], [0], color=PALETTES[nm], lw=3) for nm in NM_ORDER]
...
for event in ["stimOn", "feedback"]:
    ...
    fig.suptitle(f"{event} - PSTH mean ± SEM", fontsize=13)
    # fig.legend(PALETTES.keys(), loc="top right", ncol=4, frameon=False)
    fig.legend(LEGEND_HANDLES, NM_ORDER, frameon=False)
    plt.tight_layout(rect=[0, 0.05, 1, 0.95])
    # Save figure (PNG + PDF)
    save_base = os.path.join(BASE_DIR, f"PSTH_{event}_4NMs_correct_incorrect")
    plt.savefig(f"{save_base}.png", dpi=300, bbox_inches="tight")
    plt.savefig(f"{save_base}.pdf", dpi=300, bbox_inches="tight")
