...
RESPONSES_FNAME = 'responses_2025-11-05-10h32.pqt'
...
responses = []
exceptions_log = []
for idx, session in tqdm(df_sessions.iloc[20:25].iterrows(), total=len(df_sessions)):
    ...
    try:  # check the signal has no sampling issues
        ...
        exception_info = {  # collect exception info
            ...
            'exception_message': str(e),
            'traceback': traceback.format_exc(),
            'description': "photometry acquisition error"
        }
        exceptions_log.append(exception_info)
        pprint(exception_info)
        continue
